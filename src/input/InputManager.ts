...
import type { InputSource, PollContext, SchemeId, SourceSample } from './sources.js';
import type { SettingsStore } from './settings.js';
import { TouchSource } from './TouchSource.js';

export interface InputEvents extends Record<string, unknown> {
  schemeChange: { scheme: SchemeId; label: string };
}

/** Supplied by the game: the nearest live enemy to a point, for auto-aim. */
...
export class InputManager {
   ...
   */
  update(playerScreen: Vec2, playerWorld: Vec2): Intent {
    ...
    let chosen: SourceSample | null = null;

    for (const source of this.sources) {
      if (!source.available()) continue;
      const sample = source.poll(ctx);
      if (sample.active) this.lastActive.set(source.id, now);
      if (source.id === this.scheme) chosen = sample;
  ...
  triggerRumble(weakIntensity: number, strongIntensity: number, durationMs: number): void {
    if (!this.settings.current.vibration) return;
