...
export type SchemeId = 'kbm' | 'pad' | 'touch';

/** Where the player is right now, in *screen* pixels — needed for pointer aiming. */
export interface PollContext {
  playerScreenX: number;
  playerScreenY: number;
  settings: Readonly<InputSettings>;
}

export interface SourceSample {
  moveX: number;
  moveY: number;
  /** Absolute weapon angle in radians, or null if this source is not aiming. */
  aim: number | null;
  firing: boolean;
  /** Held state; the manager converts this into an edge. */
  ability: boolean;
  /** True if the user has touched this device since the last poll. */
  active: boolean;
}
...
export const EMPTY_SAMPLE: SourceSample = {
  moveX: 0,
  moveY: 0,
  aim: null,
  firing: false,
 ...
 */
export function applyDeadzone(x: number, y: number, deadzone: number): { x: number; y: number; mag: number } {
  ...
  if (mag === 0 || mag <= deadzone) return { x: 0, y: 0, mag: 0 };
