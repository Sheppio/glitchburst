...
export class GamepadNavigator {
  ...
  private tick = (): void => {
    ...
    if (nav.connected) {
      ...
      if (nav.menu && !this.modalOpen()) this.lockFocus();
    }
  ...
  private focusables(): HTMLElement[] {
    const nodes = this.navScope().querySelectorAll<HTMLElement>(
  ...
  private openModals(): HTMLElement[] {
    return [...this.root.querySelectorAll<HTMLElement>('[data-nav-modal]:not([hidden])')].filter(isVisible);
  }

  /** The topmost open modal, or the whole overlay when none is open. */
  private navScope(): ParentNode {
   ...
   */
  private nudgeRange(el: HTMLInputElement, direction: number): void {
    ...
    if (el.value === before) return; // already at the end of its travel
   ...
   */
  private cycleSelect(el: HTMLSelectElement, direction: number): void {
...
function isRange(el: HTMLElement): el is HTMLInputElement {
