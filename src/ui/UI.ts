...
export interface UICallbacks {
  ...
  /**
   * Callsign or program changed in the staging area.
   *
   * Separate from `onDeploy` because the room is already joined by then: this
...
/** A numeric setting, rendered as a range input with a live read-out. */
interface SliderDef {
  key: keyof typeof RANGES & keyof InputSettings;
  title: string;
  step: number;
  detail: string;
...
const percent = (value: number): string => (value <= 0 ? 'MUTED' : `${Math.round(value * 100)}%`);
...
const SLIDERS: SliderDef[] = [
  ...
  },
  {
    ...
    step: 0.01,
    detail:
      'A fixed 0.15 per-axis filter is always applied to stop stick drift on worn controllers. This adds radial deadzone on top.',
...
export class UI {
  ...
  private keyboardTarget: HTMLInputElement | null = null;
  ...
  constructor(
    ...
    document.addEventListener('gb:text-entry', (e) => {
      ...
      if (field instanceof HTMLInputElement) this.openKeyboard(field);
    });
  ...
  private callsignFields(): HTMLInputElement[] {
  ...
  private buildLobbyClasses(): void {
    ...
    select.addEventListener('change', () => {
      if (!isClassId(select.value)) return;
      this.sfx.click();
  ...
  private buildSliders(): void {
    ...
    list.replaceChildren(
      ...SLIDERS.map((def) => {
        ...
        if (def.key === 'sfxVolume') range.addEventListener('change', () => this.sfx.click());
  ...
  private syncSettings(): void {
    ...
    for (const def of SLIDERS) {
      const row = this.root.querySelector<HTMLElement>(`.slider-row[data-key="${def.key}"]`);
      if (!row) continue;
      ...
      if (readout) readout.textContent = def.format(value);
  ...
  private syncInGameMenu(): void {
    ...
    if (overGame === this.menuVisible) return;
  ...
  private openKeyboard(field: HTMLInputElement): void {
  ...
  private typeKey(char: string | null): void {
    const field = this.keyboardTarget;
    if (!field) return;
