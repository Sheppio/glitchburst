/**
 * Class glyphs for the DOM.
 *
 * The in-game chassis cannot be reused here: every class draws the *same*
 * sprite and differs only in colour and radius, which is fine at arm's length
 * in a moving arena and useless in a roster row where four programs sit
 * stacked. So each class gets a mark of its own, built from the one thing the
 * sprite does establish — a white disc inside a coloured ring — with the
 * class's behaviour drawn inside it:
 *
 *   Overclocker  a lance straight through the ring — the beam pierces
 *   Fireman      a spread fanning out of the muzzle — wide, short, loud
 *   Glitcher     a second ring offset behind the first — the decoy
 *   Encoder      a cross — the field that repairs whoever stands in it
 *
 * Everything is stroked in `currentColor`, so the caller sets the class colour
 * with CSS and the glyph inherits it. That keeps the colour in one place: the
 * roster row already paints its border from `cssColour`, and a second copy
 * baked into the markup would be the copy that goes stale.
 */
import type { ClassId } from '../types.js';

/** Drawn inside a 24×24 box on a ring of radius 8 centred at (12, 12). */
const MARKS: Record<ClassId, string> = {
 ...
 */
export function classIconSvg(id: ClassId): string {
  return (
    `<svg class="class-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false" ` +
