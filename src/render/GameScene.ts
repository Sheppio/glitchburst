...
export interface GameSceneInit {
  ...
  /** The local player was hit. Drawn by the HUD, which owns the screen edge. */
  onDamage: () => void;
}
...
interface Bullet {
  ...
  vy: number;
  life: number;
  /** Lifetime it started with, so the tail end can be faded out. */
  maxLife: number;
  damage: number;
  pierce: number;
  radius: number;
...
export class GameScene extends Phaser.Scene {
  ...
  private dangerVignette: Phaser.GameObjects.Image | null = null;
  ...
  private autopilot(): { x: number; y: number; ability: boolean } {
    ...
    const chips: Array<{ x: number; y: number; ttl: number }> = [];
    for (const chip of this.progression.chips.items) {
      if (chip.active) chips.push({ x: chip.x, y: chip.y, ttl: chip.ttl });
    }

    const powerUps: Array<{ x: number; y: number; ttl: number }> = [];
    for (const powerUp of this.progression.powerUps.items) {
      if (powerUp.active) powerUps.push({ x: powerUp.x, y: powerUp.y, ttl: powerUp.ttl });
    }
  ...
  private showHeat(view: EnemyView, age: number): void {
    ...
    if (view.heat >= 0 && step > view.heat && step === HEAT_WARN) this.cfg.sfx.enrage();
    view.heat = step;
    ...
    view.sprite.setTint(
      (255 << 16) | (Math.round(255 - heat * 70) << 8) | Math.round(255 - heat * 165),
    );
   ...
   */
  private wheelZoom(deltaY: number, event?: WheelEvent): void {
    ...
    const pixels =
      mode === 1 ? (deltaY * ZOOM.wheelNotch) / 3
  ...
  private showDanger(): void {
    ...
    if (this.downedFor > 0 || this.gameOver || this.paused) {
      if (wash.alpha !== 0) wash.setAlpha(0);
      return;
    }
