...
export interface Marker {
  /** Screen-space, relative to the top-left of the view. */
  x: number;
  y: number;
  /** Radians, pointing from the screen centre out toward the target. */
  angle: number;
  /** World distance from the centre of the view, for labelling or fading. */
  distance: number;
}
 ...
 * from.
 */
export function edgeMarker(target: Vec, view: ViewRect, margin: number): Marker | null {
  ...
  if (sx >= 0 && sx <= view.width && sy >= 0 && sy <= view.height) return null;
