 ...
 */
export const LEVEL_COLOURS: readonly number[] = [
  0x7c3aed, // 1 violet
  0x2563eb, // 2 blue
  0x0891b2, // 3 cyan
  0x16a34a, // 4 green
...
export const LEVEL_CSS: readonly string[] = LEVEL_COLOURS.map(
 ...
 */
export function levelRewardScale(level: number): number {
 ...
 * the scoreboard.
 */
export function killScore(baseScore: number, level: number): number {
  return Math.round(baseScore * levelRewardScale(level));
}

/** The level a wave spawns by default, before spread. */
export function baseLevelForWave(wave: number): number {
 ...
 */
export function rollLevel(wave: number, random: () => number = Math.random): number {
  const base = baseLevelForWave(wave);
  const roll = random();
  if (roll > LEVELS.spreadChance) return base;
