...
export interface TargetCandidate {
  ...
  x: number;
  y: number;
  /** Current health. The whole point of the exercise. */
  hp: number;
}
...
export interface TargetingContext {
  fromX: number;
  fromY: number;
  /** Where the barrel points now, in radians. */
  facing: number;
  /** Turn rate in radians per second. */
  turnRate: number;
  /** Effective damage per second against a single target. */
  dps: number;
  bulletSpeed: number;
  /** How far a round travels before expiring. */
  weaponRange: number;
  /** Acquisition range; nothing beyond this is considered at all. */
  maxRange: number;
 ...
 */
export function targetScore(candidate: TargetCandidate, ctx: TargetingContext): number {
  ...
  const rangePenalty =
    ctx.bulletSpeed > 0 ? (overshoot / ctx.bulletSpeed) * TARGETING.outOfRangeWeight : 0;
