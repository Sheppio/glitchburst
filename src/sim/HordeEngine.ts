...
export interface StepResult {
  events: HordeEvent[];
  /** Enemies removed this step, with the attacker credited for the kill. */
  kills: Array<{ id: EnemyId; kind: EnemyKind; x: number; y: number; score: number; attacker: string }>;
}
...
export class HordeEngine {
  readonly enemies = new Map<EnemyId, Enemy>();
  ...
  private roster: EnemyKind[] = [EnemyKind.GlitchBug];
   ...
   */
  adopt(
    snapshots: Iterable<{ id: EnemyId; x: number; y: number; kind: EnemyKind; level?: number; hp: number }>,
    wave: number,
  ): void {
    this.enemies.clear();
    let highest = 0;
   ...
   */
  reportDamage(enemyId: EnemyId, amount: number, attacker: string): void {
    if (!this.enemies.has(enemyId) || !(amount > 0)) return;
    const existing = this.pending.get(enemyId);
    if (existing) existing.amount += amount;
    else this.pending.set(enemyId, { amount, attacker });
  }

  /** Fireman ability: push everything nearby away and stun it. */
  applyShockwave(x: number, y: number, radius: number, stunSec: number, impulse: number): void {
  ...
  step(dt: number, targets: readonly AiTarget[]): StepResult {
    const result: StepResult = { events: [], kills: [] };
  ...
  private resolveDamage(result: StepResult): void {
    ...
    for (const [id, dmg] of this.pending) {
      ...
      if (e.hp <= 0) {
        ...
        for (let n = 0; n < (def.splitInto ?? 0); n++) {
          if (this.enemies.size >= HORDE.maxEnemies) break;
          const angle = (Math.PI * 2 * n) / (def.splitInto ?? 1);
          this.spawnAt(
            EnemyKind.GlitchBug,
            e.x + Math.cos(angle) * (def.radius + 6),
            e.y + Math.sin(angle) * (def.radius + 6),
            e.level,
          );
        }
      }
  ...
  private rollRoster(): EnemyKind[] {
    ...
    if (unlocked.length <= 2) return unlocked;

    const picks = new Set<EnemyKind>(debut);
    ...
    if (fodder.length) picks.add(fodder[Math.floor(Math.random() * fodder.length)]!);
  ...
  private rollKind(): EnemyKind {
    ...
    for (let i = 0; i < this.roster.length; i++) {
      roll -= weights[i]!;
      if (roll <= 0) return this.roster[i]!;
    }
  ...
  private spawnAt(kind: EnemyKind, x: number, y: number, level = rollLevel(this.wave)): Enemy | null {
    if (this.enemies.size >= HORDE.maxEnemies) return null;
    const def = ENEMY_DEFS[kind];
  ...
  private spawn(kind: EnemyKind, targets: readonly AiTarget[]): Enemy | null {
    if (this.enemies.size >= HORDE.maxEnemies) return null;
    const def = ENEMY_DEFS[kind];
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const tooClose = targets.some(
        (t) => t.alive && dist2(x, y, t.x, t.y) < HORDE.minSpawnDistance * HORDE.minSpawnDistance,
      );
      if (!tooClose) break;
    }
   ...
   */
  private pickTarget(e: Enemy, targets: readonly AiTarget[]): AiTarget | null {
   ...
   */
  private separate(e: Enemy, radius: number): void {
    const cx = (e.x / 72) | 0;
    const cy = (e.y / 72) | 0;
    let pushX = 0;
    let pushY = 0;
