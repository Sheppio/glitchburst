...
/** What one client contributes to the group total. */
export interface PlayerStats {
  /** Individual rounds, not trigger pulls: a shotgun blast is seven. */
  shots: number;
  chips: number;
  powerUps: number;
  /** Times this player came back. */
  reboots: number;
}

/** What the host reports about the run as a whole. */
...
export const EMPTY_PLAYER_STATS: PlayerStats = { shots: 0, chips: 0, powerUps: 0, reboots: 0 };
export const EMPTY_ROOM_STATS: RoomStats = { kills: 0, wave: 0, seconds: 0, score: 0 };

export function sumPlayerStats(all: Iterable<PlayerStats>): PlayerStats {
 ...
 */
export function accuracy(kills: number, shots: number): number {
  if (shots <= 0) return 0;
