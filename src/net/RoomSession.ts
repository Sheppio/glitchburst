...
export interface PeerRecord {
  ...
  name: string;
  cls: string;
  /** Palette id this peer asked for; `resolvedColours` settles the clashes. */
  colour: string;
  claimsHost: boolean;
  lastSeen: number;
}
...
export class RoomSession {
  ...
  constructor(
    ...
    private name: string,
    private cls: ClassId,
    private colour: string = DEFAULT_COLOUR,
  ) {}
   ...
   */
  setIdentity(name: string, cls: ClassId, colour: string = this.colour): void {
    if (name === this.name && cls === this.cls && colour === this.colour) return;
    this.name = name;
    this.cls = cls;
    this.colour = colour;
   ...
   */
  resolvedColours(): Record<PlayerId, string> {
    const claims = [{ id: this.playerId, colour: this.colour }];
    for (const peer of this.peers.values()) claims.push({ id: peer.id, colour: peer.colour });
  ...
  private onPresence(id: PlayerId, payload: string): void {
    ...
    if (record.claimsHost) this.resolveHostClaim(id);
  }

  /** Somebody else says they are host. Decide whether that outranks us. */
  private resolveHostClaim(id: PlayerId): void {
    if (this._isHost) {
  ...
  private tick(): void {
    ...
    if (gap > NET.stallForgivenessMs) {
      for (const peer of this.peers.values()) peer.lastSeen = now;
  ...
  private evaluateHost(reason: 'initial' | 'election'): void {
    ...
    if (!wasHost && this._hostId === winner) return;

    this._isHost = false;
    this._hostId = winner;
    this.events.emit('hostChange', { hostId: winner, isHost: false, reason });
    if (wasHost) this.announcePresence();
  }
}
