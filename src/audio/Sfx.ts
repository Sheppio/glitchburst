...
export class Sfx {
   ...
   */
  enemyShot(): void {
    if (!this.bus.throttle('enemyShot', 0.07)) return;
   ...
   */
  enrage(): void {
    if (!this.bus.throttle('enrage', 4)) return;
   ...
   */
  alarm(ratio: number): void {
    if (!Number.isFinite(ratio) || ratio > 0.3 || ratio <= 0) return;
    // 0.92s of quiet at the threshold, 0.42s at the edge of death.
    const urgency = 1 - ratio / 0.3;
    if (!this.bus.throttle('alarm', 0.92 - urgency * 0.5)) return;
