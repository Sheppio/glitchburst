 ...
 */
function pathClearance(view, tx, ty) {
 ...
 */
function chooseLoot(view, nearest) {
    const candidates = [];
    const consider = (pickup, value, reach, radius, weight) => {
        ...
        if (pickup.ttl !== undefined && view.moveSpeed && view.moveSpeed > 0) {
            if (travel / view.moveSpeed > pickup.ttl * AUTOPILOT.ttlMargin)
                return;
        }
        candidates.push({
            x: pickup.x,
    ...
    for (const powerUp of view.powerUps ?? []) {
        consider(powerUp, AUTOPILOT.powerUpValue, AUTOPILOT.powerUpReach, AUTOPILOT.powerUpRadius, AUTOPILOT.powerUpWeight);
    }
 ...
 */
function escapeHeading(view, loot) {
 ...
 */
export function smoothHeading(previous, next, dt) {
 ...
 */
export function autopilotMove(view) {
    ...
    for (const enemy of view.enemies) {
        const d = dist(view.x, view.y, enemy.x, enemy.y);
        if (d >= AUTOPILOT.dangerRadius || d <= 0)
            continue;
    ...
    const { width, height } = view.world;
    if (view.x < AUTOPILOT.wallMargin)
        vx += (1 - view.x / AUTOPILOT.wallMargin) * AUTOPILOT.wallWeight;
    if (view.x > width - AUTOPILOT.wallMargin) {
        vx -= (1 - (width - view.x) / AUTOPILOT.wallMargin) * AUTOPILOT.wallWeight;
    }
    if (view.y < AUTOPILOT.wallMargin)
        vy += (1 - view.y / AUTOPILOT.wallMargin) * AUTOPILOT.wallWeight;
