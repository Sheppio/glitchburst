...
const SLIDERS = [
    ...
    },
    {
        ...
        title: 'Camera zoom',
        step: 0.05,
        detail: 'Pull back to see more of the arena, or close in for a bigger picture. Worth pulling back on a phone, where the default slice is a keyhole.',
...
const TOGGLES = [
    ...
    },
    {
        key: 'autoMove',
        title: 'Auto-move',
        detail: 'The client drives itself — backs off when crowded, closes in when out of range, collects chips. With auto-aim and auto-fire it plays hands-off, which is how you get a second body into a room to test multiplayer. Any real input takes over instantly.',
    },
    {
...
function readStoredColour() {
    try {
        const saved = localStorage.getItem(COLOUR_KEY);
        if (saved && isColourId(saved))
            return saved;
    }
    catch {
        /* storage unavailable */
...
export class UI {
    ...
    identityTimer = 0;
    selectedColour = readStoredColour();
    /**
    ...
    announceIdentity(now = false) {
        ...
        const send = () => this.callbacks.onIdentity(this.callsign, this.selectedClass, this.selectedColour);
        if (now)
            send();
        else
    ...
    setLobby(members, isHost) {
        ...
        if (mine && mine.colour !== this.selectedColour)
            this.selectColour(mine.colour);
        else
    ...
    updateHud(s) {
        ...
        if (s.rebootsLeft !== null)
            this.text('hud-reboots', String(s.rebootsLeft));
     ...
     */
    buildColourGrids() {
        for (const where of ['class', 'lobby']) {
            const grid = this.el(`colour-grid-${where}`);
            grid.replaceChildren(...PALETTE.map((swatch) => {
    ...
    syncColourGrids() {
        for (const button of this.root.querySelectorAll('.colour-swatch')) {
